...
public class Personal {
	...
	public static void getMatchIds() {
		...
		while (matchesLeft > 0) {
			...
			/*
			 * String query = "SELECT * FROM matches WHERE match_id = ?"; try {
			 * PreparedStatement ps = (PreparedStatement)
			 * con.prepareStatement(query); ps.setInt(1,
			 * matchIds.get(matchIds.size() - 1)); ResultSet rs =
			 * ps.executeQuery(); if (rs.next()) { return; } } catch
			 * (SQLException e) { System.out.println(
			 * "Failed to check database of match_id record in getMatchIds()");
			 * e.printStackTrace(); }
			 */
		}
